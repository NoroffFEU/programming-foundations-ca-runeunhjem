...
var tv = {
  ...
  brand: { // Nested Object
    ...
    hq: {
      officePark: "Samsung Digital City, Suwon, South Korea", // String
  ...
  hdrIsOn: true, // Boolean
  isSmartTv: true, // Boolean
  tvIsOn: true, // Boolean
  ...
  adjustVolume: function (volume) {
    // Functions within an object are also called methods
  ...
  changeSource: function (input) {
    // Functions within an object are also called methods
    if (
      this.availableInputs.includes(input) ||
      this.availableApps.includes(input)
    ) {
      // ****** The this keyword points to property's parent (tv) ***********
...
hdrButton.style.backgroundColor = "green"; // Setting initial HDR button state
// *******************************************
...
function resetSourceBackground() {
  ...
  activeLogo.innerHTML = `<img src=""></div>`;
}
