...
var tv = {
  ...
  // FUNCTION TO CHANGE COLOR OF ACTIVE SOURCE BUTTON
  
  activeTvSourceColor:  function currentSource() {
  if(tv.activeTvSource === tv.apps) {
    console.log("====> Active TV Source is", this.activeTvSource);
    tv.availableApps.forEach(apps => var tv.apps = apps);
    var activeTvSource = this.activeTvSourceColor;
    console.log("====> Active TV Source", tv.activeTvSource);
    pcButton.style.backgroundColor = "#cdcdcd";
    pcButton.style.color = "#000";
    pcButton.style.fontWeight = "600";
  }
}
...
tv.activeTvSourceColor("PC");

// tv.availableApps.forEach(apps => console.log(apps));